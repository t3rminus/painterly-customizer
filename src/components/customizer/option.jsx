...
export function CustomizerOption({ option, group }) {
  ...
  return (
    <label key={option.id} className="relative cursor-pointer">
      ...
      />
      <div className="flex gap-2 p-2 rounded-sm items-center text-sm leading-tight peer-checked:bg-base-200">
        <span className="w-3 h-3 shrink-0 border border-current rounded-full in-peer-checked:bg-current" />
        <img
          src={option.preview}
